...
function execute(){
  ...
  request('https://wrapapi.com/use/CataclysmX/check/test/0.0.1?wrapAPIKey=PGjrMdMfucZIFtxuus7ogmCpxdWHBXmR', function (error, response, body) {
    ...
    if (typeof body2 !== 'undefined')
    {
      ...
      {
        console.log('New ! ');
            request('https://smsapi.free-mobile.fr/sendmsg?user='+ process.env.ID +'&pass='+ process.env.Pass +'&msg=Vite+%21+%0D%0AWallah+y+a+du+Nouveau+%21%0D%0A%0A'+ encodeURIComponent(body2) +'%0A%0Ahttps%3A%2F%2Fwww.saintepass.fr%2Foffres-a-saisir%3Fpage%3D1', function (error, response, bodyRep) {
              //console.log('error:', error); // Print the error if one occurred
              //console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
              console.log(bodyRep);});
        i = 0;
